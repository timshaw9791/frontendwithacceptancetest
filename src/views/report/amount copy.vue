      ...
      <bos-tabs :option="['contrast']" :layoutRatio="[1,3]" :contrastKey="['slot1', 'slot2']">
        ...
        <div slot="slot2" class="safety-body-top">
          ...
          <div style="safety-body-t" v-else-if="show=='genres'">
            <div
              style="float:left"
            >总计 装备大类：{{this.title}} 当前库存：{{addNum(1)}}件 可用数：{{addNum(2)}}件 领用数：{{addNum(3)}}件 当前库存总价(￥)：{{addNum(4)}}</div>
            <div style="float:right">
              <define-input label="小类" v-model="search2"></define-input>
            </div>
          </div>
          <div style="safety-body-t" v-else-if="show=='singlePolice'">
            <div
              style="float:left"
            >总计 装备大类：{{this.title}} 当前库存：{{this.equipArg.reduce((v,k)=>v+k.totalCount,0)}}件 当前库存总价(￥)：{{addNum(4)}}</div>
          </div>
          <div style="safety-body-t" v-else-if="show=='singlePoliceCategory'">
            <div
              style="float:left"
            >总计 装备小类：{{this.title}} 当前库存：{{this.equipArg.reduce((v,k)=>v+k.totalCount,0)}}件 当前库存总价(￥)：{{addNum(4)}}</div>
            <div style="float:right">
          ...
          <div style="safety-body-t" v-else-if="show=='category'">
            <div
              style="float:left"
            >总计 装备小类：{{this.title}} 当前库存：{{addNum(1)}}件 可用数：{{addNum(2)}}件 领用数：{{addNum(3)}}件 当前库存总价(￥)：{{addNum(4)}}</div>
            <div style="float:right">
              <define-input label="装备参数/供应商" v-model="search2"></define-input>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    addNum(item) {
      ...
      if (item == 2)
        return this.equipArg.reduce((v, k) => v + k.inHouseCount, 0);
      if (item == 3)
        return this.equipArg.reduce((v, k) => v + k.receiveUseCount, 0);
