    ...
    <div class="safety-body" >
         <bos-tabs  :option="['contrast']" :layoutRatio="[1,3]" :contrastKey="['slot1', 'slot2']" >
            <div slot="slot1"  class="safety-body-top">
                ...
                <div style="height:80%">
                    <define-tree @clickNode="clickNode" :search="search" :accordion="true" :data="tree.treeData" :expandAll="false" :options="options" @nodeClick="clickNode"></define-tree>
                </div>
    ...
    export default {
        ...
        methods: {
            ...
            rate(data){
                ...
                return (data.count/(data.totalCount+data.commonStock)*100).toFixed(2)
                }else return 0
            },
            policeRate(data){
            ...
            searchCategory(item){
               if(item=='CATEGORY'||item=='GENRE'){
                   console.log("触发");
                   findEquipLossStatistics({categorys:3,id:this.id,level:item,search:this.search2}).then(res=>{
                        this.equipArg = res
                        this.equipArg.forEach(item=>{
                            item.equipArgs=`${item.name}(${item.model})`
                        })
                    ...
                    cabinetLossCategories(this.id,this.search2).then(res=>{
                        this.equipArg = res
                        this.paginator.totalPages = res.totalPages;
                        this.paginator.totalElements = res.totalElements;
                    })
            ...
            handler(newval){
                ...
                if(this.show=='category'||this.show=='genres'){
                    ...
                    if(this.show=='category')searchI='CATEGORY'
                     findEquipLossStatistics({categorys:3,id:this.id,level:searchI,search:newval}).then(res=>{
                        this.equipArg = res
                        this.equipArg.forEach(item=>{
                            item.equipArgs=`${item.name}(${item.model})`
                        })
