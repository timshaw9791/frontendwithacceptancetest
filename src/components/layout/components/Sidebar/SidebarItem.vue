    ...
    <div class="menu-wrapper">
        <template v-for="item in routes" v-if="!item.hidden&&item.children">
            ...
            <router-link v-if="hasOneShowingChildren(item.children) && !item.children[0].children&&!item.alwaysShow"
                         ...
                         :data-a="item.children[0].meta.title"
            >
            ...
            <el-submenu v-else :index="item.name||item.path" :key="item.name" :data-a="item.meta.title">
                <template slot="title">
                ...
                <template v-for="child in item.children" v-if="!child.hidden">
                    ...
                    <router-link v-else :to="item.path+'/'+child.path" :key="child.name" :data-a="child.meta.title">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // hasPermission(roles) {
            //   this.$router.options.routes.forEach((role) => {
            //     if (role['role'] && role['role'].includes(roles)) {
            //       role['children'].forEach((value, index) => {
            //         if (value['role']) {
            //           if (value['role'].includes(roles)) {
            //           } else {
            //             role['children'].shift(index);
            //           }
            //         }
            //       })
            //     }
