    ...
    <div class="apply-process">
        ...
        <div class="apply-process-container">
            <div class="apply-process-top" data-test="action_box" v-if="status.tableOrUniversalFlag">
                ...
                <base-button name="读取数据" :disabled="!select.selected"></base-button>
                <process-select label="硬件选择" v-model="select.selected" :selectList="select.handWareList"></process-select>
            </div>
            <div class="apply-process-body">
                ...
                <text-input label="备注" v-model="reMarks" class="remarks"></text-input>
            </div>
        </div>
    </div>
</template>
